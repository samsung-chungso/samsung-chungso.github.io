...
L2 distance 공식 안의 제곱근을 풀어줍니다. 힌트 정보에 따라 뺄셈보다는 곱이 잔뜩 들어간 공식으로 바꿀 수 있습니다.   
        $$(I_1^P-I_2^P)^2 = (I_1^P)^2 - 2*I_1^P*2I_2^P + (I_2^P)^2$$
        $$(I_1^P-I_2^P)^2 = (I_1^P)^2 + I_2^P(I_2^P - 2*I_1^P)$$
        $$(I_1^P-I_2^P)^2 = \sqrt{\displaystyle\sum_{p}(I_1^P)^2} - \sqrt{\displaystyle\sum_{p}2*I_1^P*2I_2^P} + \sqrt{\displaystyle\sum_{p}(I_2^P)^2}$$  
...
columns 하나하나가 Train 이미지 하나를 의미하므로 column 한 줄은 Train 이미지와 
