...
### Cross-validation
Machine Learning에서 배운 내용과 동일하게 training data의 크기가 작다면, validation set을 많이 만들기 위해서 
### In practice
...
## Linear Classifier
앞으로 배울 CNN의 구성에 많은 영향을 끼친 Linear Classifier는 레고처럼 쌓을 수 있는 layer를 가집니다. 이 모델은 train dataset은 더이상 필요 없어졌고, 계산에 필요한 f(x)만 필요할 뿐입니다. 
